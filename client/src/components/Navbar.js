...
import { logout } from '../redux/authSlice';  // FIXED
...
function Navbar() {
  ...
  const user = useSelector(state => state.auth.user);  // FIXED: From auth
  const cartItems = useSelector(state => state.cart.items);
  const itemCount = cartItems.reduce((sum, item) => sum + item.quantity, 0);

  // Dark Mode State (if you have it)
  // ... (keep your dark mode code if present)
      ...
      <div className="container">
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav ms-auto">
            ...
            {user?.isAdmin && <li className="nav-item"><Link className="nav-link" to="/admin"><FaUserCog /> Admin</Link></li>}
            {user ? (
